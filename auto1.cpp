...
// - auto와 auto&&는 테플릿 파라미터 T, T&& 와 같다. (universial reference or forwarding reference)
...
int main()
{
    ...
    //1. 오른쪽이 참조형인 경우 참조는 무시된다.
    //2. const와 volatile 등이 있을 경우에도 해당 키워드는 무시된다.

    const int& const_ref = x;
    auto something = const_ref;  //something의 타입은 int 다.

    x = 10;

    cout << "something: " << something << endl; //0
    cout << "const_ref: " << const_ref << endl; //10
    cout << "x: " << x << endl;                 //10
    ...
    //주의할 점은 ...
    //아래 구문은 동일합니다. 하지만 두 구문의 사용법이 항상 같다고 생각하면 안됩니다.
    //decltype에 대해서는 다음에 조금 더 상세히 다루도록 하겠습니다.
    auto mul1 = x * y; //int x, double y; doulbe mul1
